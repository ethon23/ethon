...
app.post('/api/payment', rateLimitMiddleware('payment', 8, 300), (req, res) => {
  const { name = '', email = '', phone = '', service = '', amount = 0, currency = 'USD', method = '', message = '' } = req.body || {};
  const tName = String(name).trim();
  const tEmail = String(email).trim();
  const numAmount = Number(amount) || 0;
  const tMethod = String(method).trim();

  if (!tName || !tEmail.includes('@') || numAmount <= 0 || !tMethod) {
    return res.status(400).json({ error: 'Name, email, amount and payment method are required.' });
  }

  const curr = String(currency).toUpperCase() === 'BDT' ? 'BDT' : 'USD';
  const now = new Date().toISOString();
  const db = loadDb();
  ...
  const gatewayUrlMap = {
    STRIPE: String(db.settings?.paymentStripeUrl || '').trim(),
    PAYPAL: String(db.settings?.paymentPaypalUrl || '').trim(),
    PAYONEER: String(db.settings?.paymentPayoneerUrl || '').trim(),
    'GOOGLE PAY': String(db.settings?.paymentGooglePayUrl || '').trim(),
    'BINANCE PAY': String(db.settings?.paymentBinanceUrl || '').trim(),
    BKASH: String(db.settings?.paymentBkashUrl || '').trim()
  };
  const voucherNo = `ETH-PV-${now.slice(0,10).replace(/-/g,'')}-${String(msgObj.id).slice(-6).toUpperCase()}`;
  msgObj.voucherNo = voucherNo;
  msgObj.gatewayUrl = gatewayUrlMap[tMethod.toUpperCase()] || '';
...
app.get('/api/payment/:id', (req, res) => {
  ...
  if (String(req.params.id) === demoId) {
    return res.json({
      ...
      service: 'UI/UX Design', amount: 750, currency: 'USD', paymentMethod: 'STRIPE',
...
app.post('/api/chat/start', rateLimitMiddleware('chat-start', 10, 300), (req, res) => {
  ...
  if (!conv) {
    ...
    if (autoMessage) {
      conv.messages.push({ id: newId(), from: 'admin', channel: 'chat', text: autoMessage, createdAt: now, automated: true });
    }
  } else {
    conv.source = 'chat';

...
app.post('/api/payment', rateLimitMiddleware('payment', 8, 300), (req, res) => {
  ...
  const gatewayUrl = paymentGatewayUrl(db, tMethod, curr, numAmount);
  const voucherNo = `ETH-PV-${now.slice(0,10).replace(/-/g,'')}-${String(msgObj.id).slice(-6).toUpperCase()}`;
  msgObj.voucherNo = voucherNo;
...
app.post('/api/chat/start', rateLimitMiddleware('chat-start', 10, 300), (req, res) => {
  ...
  if (!conv) {
    ...
    // If an older conversation never received the configured automatic first reply,
    // send it now so returning visitors are not left without the greeting.
    const autoMessage = String(db.settings?.chatAutoMessage || '').trim();
    const hasAutoReply = Array.isArray(conv.messages) && conv.messages.some(m => m && m.from === 'admin' && m.automated === true);

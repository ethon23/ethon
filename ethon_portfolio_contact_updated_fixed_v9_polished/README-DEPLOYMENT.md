...
The server can automatically email the client after a payment request is created. On Railway, add these variables:
- `RESEND_API_KEY` — your Resend API key
- `RESEND_FROM_EMAIL` — a verified sender, e.g. `Ethon <payments@yourdomain.com>`
The request is still saved and added to the client conversation if email delivery is unavailable.

## Currency-aware gateway links
The checkout accepts `USD`, `GBP`, `EUR`, and `BDT`. For real merchant charging in the selected currency, configure currency-specific gateway URLs in Admin → Payments → Currency-specific checkout URLs (JSON). Example: `{"STRIPE":{"USD":"...","GBP":"...","EUR":"...","BDT":"..."}}`. If no currency-specific URL is configured, the normal gateway URL is used; the selected currency is still stored on the request and voucher.
